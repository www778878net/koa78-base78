...
async function runSimpleWorkflow() {
    ...
    // 为任务添加执行函数
    task1.taskFunction = async (input) => {
        console.log('  执行任务1...');
        return { result: '任务1完成', value: 100 };
    };

    task2.taskFunction = async (input) => {
        console.log('  执行任务2...');
        return { result: '任务2完成', value: 200 };
    };

    task3.taskFunction = async (input) => {
        console.log('  执行任务3...');
...
// 执行带Agent的工作流
async function runWorkflowWithAgent() {
    console.log('\n\n=== 带Agent的工作流测试 ===\n');
    ...
    // 设置处理器执行函数
    testHandler.setCallback(async (params) => {
...
async function main() {
    try {
        // 运行简单工作流
        const result1 = await runSimpleWorkflow();
        console.log(`\n简单工作流测试${result1 === 'completed' ? '成功' : '失败'}`);

        // 运行带Agent的工作流
        const result2 = await runWorkflowWithAgent();
