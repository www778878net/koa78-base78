...
// 1. 定义工作流处理函数
// 数据校验Handler
const dataValidationHandler = new Handler();
...
dataValidationHandler.setCallback(async (inputData: any) => {
...
childHandler.setCallback(async (inputData: any) => {
...
// 结果合并Handler
const resultMergeHandler = new Handler();
...
resultMergeHandler.setCallback(async (inputData: any) => {
...
// 3. 创建工作流实例并执行
async function runWorkflowTest(userData: any) {
    console.log('\n========================================');
    ...
    const finalResult = Object.values(workflow.get_task_results())
        .filter(result => result?.result?.finalResult)
...
(async () => {
    // 测试成人分支
    await runWorkflowTest({ name: '张三', age: 25 });

    // 测试儿童分支
    await runWorkflowTest({ name: '李四', age: 12 });

    // 测试边界情况
    await runWorkflowTest({ name: '王五', age: 18 });
})();