...
// 设置验证器回调函数
paramValidatorHandler.setCallback(async (inputData: any) => {
...
// 设置数据库查询回调函数
dbQueryHandler.setCallback(async (inputData: any) => {
...
// 创建结果格式化Handler
const resultFormatterHandler = new Handler();
...
// 设置结果格式化回调函数
resultFormatterHandler.setCallback(async (inputData: any) => {
...
// 创建工作流定义
const workflowDef = new Workflow();
...
// 执行工作流
(async () => {
    const result = await workflowDef.execute(agent);
    console.log('\n=== 工作流执行结果 ===');
    ...
    // 测试失败情况
    console.log('\n=== 测试失败情况 ===');
    const workflowDef2 = new Workflow();
    ...
    const result2 = await workflowDef2.execute(agent);
