...
async function runAgentWorkflowDemo() {
    ...
    try {
        // 执行SID验证任务
        console.log('\n📝 执行SID验证任务...');
        const sidValidationResult = await authServiceAgent.executeHandlerByCapability('validation', 'sid-validation', {
            sid: upInfoAgent.sid
        });
        ...
        // 执行权限验证任务
        console.log('\n📝 执行权限验证任务...');
        const permissionResult = await authServiceAgent.executeHandlerByCapability('validation', 'permission-validation', {
            userId: 'test_user'
        });
