...
const statsMiddleware = (ctx, next) => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
    ...
    // const doc = {
    //     apisys: apisys,            // API version
    //     apimicroroapimicroicro,        // API system
    ...
    // };
    // try {
    //     // Elasticsearch index and document ID are based on the method, apimicroro, and apiobj
    //     const index = 'sys_nodejs-main';
    //     const id = `${apifun}-${apimicroro}-${apiobj}-${apisys}`;
...
function setupRoutes(app) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        router.all('/:apisys/:apimicroro/:apiobj/:apifun', (ctx) => tslib_1.__awaiter(this, void 0, void 0, function* () {
            try {
                const { apisys, apimicroro, apiobj, apifun } = ctx.params;
                // 判断是否是心跳API，如果是则不输出详细日志
                const isHeartbeatApi = apisys.toLowerCase() === 'apitest' &&
                    apimicroro.toLowerCase() === 'testmenu' &&
                    apiobj.toLowerCase() === 'test78' &&
                    apifun.toLowerCase() === 'test';
                // 设置标志供日志中间件使用
                ctx.isHeartbeatApi = isHeartbeatApi;
                if (!isHeartbeatApi) {
                    log.detail(`Received request for: /${apisys}/${apimicroro}/${apiobj}/${apifun}`);
                }
                if (apifun.startsWith('_') || !apisys.toLowerCase().startsWith('api') || apimicroro.toLowerCase().startsWith('dll')) {
                    log.debug(`Access denied for: /${apisys}/${apimicroro}/${apiobj}/${apifun}`);
                    ctx.status = 403;
                    ctx.body = { error: 'Access denied' };
                    return;
                }
                const ControllerClass = controllerLoader.getController(`${apisys}/${apimicroro}/${apiobj}`);
                if (!ControllerClass) {
                    log.error(`Controller not found for: ${apisys}/${apimicroro}/${apiobj}`);
                    ctx.status = 500;
                    ctx.body = { error: 'Internal Server Error', details: `Controller not found for: ${apisys}/${apimicroro}/${apiobj}` };
                    return;
                }
                if (!isHeartbeatApi) {
