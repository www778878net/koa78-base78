...
// 模拟数据库查询函数
function mockDbQuery(query, params) {
...
// 演示工作流 - 用户信息查询流程
function runUserQueryWorkflowDemo() {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        console.log('\n=== 工作流演示: 用户信息查询流程 ===\n');
        // 创建工作流实例
        ...
        // 创建任务1: 参数验证
        const validateSidTask = new task_1.Task({
            id: 'validate-sid',
            taskname: '验证SID参数',
            state: 'pending',
            priority: 1
        });
        validateSidTask.taskFunction = (input) => tslib_1.__awaiter(this, void 0, void 0, function* () {
            console.log('\n🔍 执行任务: 验证SID参数');
            const { sid } = input;
            if (!sid) {
        ...
        // 创建任务2: SID转换为用户信息
        const convertSidTask = new task_1.Task({
            id: 'convert-sid-to-user',
            taskname: 'SID转换为用户信息',
            state: 'pending',
            priority: 2
        });
        convertSidTask.taskFunction = (input) => tslib_1.__awaiter(this, void 0, void 0, function* () {
            console.log('\n🔄 执行任务: SID转换为用户信息');
            const { sid } = input;
            // 模拟SID转换逻辑
        ...
        // 创建任务3: 权限验证
        const validatePermissionTask = new task_1.Task({
            id: 'validate-permission',
            taskname: '验证用户权限',
            state: 'pending',
            priority: 3
        });
        validatePermissionTask.taskFunction = (input) => tslib_1.__awaiter(this, void 0, void 0, function* () {
            console.log('\n🛡️  执行任务: 验证用户权限');
            const { userId } = input;
            // 模拟权限验证逻辑
        ...
        // 创建任务4: 数据库查询
        const dbQueryTask = new task_1.Task({
            id: 'query-user-info',
            taskname: '查询用户信息',
            state: 'pending',
            priority: 4
        });
        dbQueryTask.taskFunction = (input) => tslib_1.__awaiter(this, void 0, void 0, function* () {
            console.log('\n🗄️  执行任务: 查询用户信息');
            const { userId } = input;
            // 使用模拟数据库查询
            const userData = yield mockDbQuery('SELECT id, username, email FROM users WHERE id = ? AND status = 1', [userId]);
        ...
        // 创建任务5: 结果格式化
        const formatResultTask = new task_1.Task({
            id: 'format-result',
            taskname: '格式化结果',
            state: 'pending',
            priority: 5
        });
        formatResultTask.taskFunction = (input) => tslib_1.__awaiter(this, void 0, void 0, function* () {
            console.log('\n📝 执行任务: 格式化结果');
            // 格式化输出，排除敏感信息
        ...
        // 创建任务6: 错误处理
        const errorHandlerTask = new task_1.Task({
            id: 'error-handler',
            taskname: '错误处理',
            state: 'pending',
            priority: 10
        });
        errorHandlerTask.taskFunction = (input) => tslib_1.__awaiter(this, void 0, void 0, function* () {
            console.log('\n❌ 执行任务: 错误处理');
...
function runAgentWorkflowDemo() {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        // 创建用户查询处理器
        const userQueryHandler = new handler_1.Handler({
            type: 'user',
            capability: 'query',
            description: '用户查询处理器',
            state: 'active'
        ...
        // 设置处理器逻辑
        userQueryHandler.setCallback((params) => tslib_1.__awaiter(this, void 0, void 0, function* () {
            console.log('\n🤖 Agent处理器执行: 用户查询');
        ...
        // 创建工作流
        const agentWorkflow = new workflow_1.Workflow({
        ...
        try {
            ...
            const result = yield agentWorkflow.execute(agent);
            console.log('\n' + '='.repeat(50));
...
function main() {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        console.log('='.repeat(60));
        // 执行工作流演示
        yield runUserQueryWorkflowDemo();
        // 执行带Agent的工作流演示
