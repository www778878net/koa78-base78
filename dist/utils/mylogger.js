 ...
 * 格式化时间戳
 */
function formatTimestamp(includeMs = false) {
    const now = new Date();
...
class MyLogger {
     ...
     */
    constructor(myname, logRetentionDays = 3, wfname = '') {
        ...
        /** 文件日志级别 */
        this.levelFile = LogLevel.INFO;
        /** 控制台日志级别 */
        this.levelConsole = LogLevel.INFO;
        /** 是否启用 detail 日志 */
        this.detailLoggerEnabled = false;
        if (!myname) {
     ...
     */
    static getInstance(myname, logRetentionDays = 3, wfname = '') {
        if (!myname) {
            throw new Error("myname cannot be empty");
        }
     ...
     * 手动设置日志级别
     */
    setupLevel(fileLevel, consoleLevel) {
    ...
    _writeToFile(message, level) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            if (level >= this.levelFile) {
                ...
                // 使用锁保护文件写入，防止并发问题
                yield this._writeLock;
                this._writeLock = (() => tslib_1.__awaiter(this, void 0, void 0, function* () {
     ...
     */
    _writeToDetailFile(message, level) {
     ...
     * 格式化日志消息
     */
    _formatLogMessage(message, level) {
        return message;
    }
    /**
     * 核心日志记录方法
     */
    _logWithPars(message, level) {
     ...
     * 记录异常日志
     */
    exception(exc, level = LogLevel.ERROR) {
